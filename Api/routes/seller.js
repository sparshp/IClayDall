...
router.delete("/:id", verifyTokenAndAuthorizationSeller, async (req, res) => {
  ...
  //GET USER
  router.get("/find/:id", verifyTokenAndAdminSeller, async (req, res) => {
    try {
...
router.get("/",  async (req, res) => {
    ...
    try {
      const users = query
        ? await Seller.find().sort({ _id: -1 }).limit(5)
        : await Seller.find();
...
router.get("/stats", verifyTokenAndAdminSeller, async (req, res) => {
    const date = new Date();
    const lastYear = new Date(date.setFullYear(date.getFullYear()-5));
